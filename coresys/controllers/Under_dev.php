...
class Under_dev extends MY_Controller {
	var $data = array();
	...
	public function index() {	
		...
		return view('pages/under_dev/index', $this->data);
	}
	...
	public function index2() {	
		...
		return view('pages/under_dev/index2', $this->data);
	}
	...
	public function get_data() {
        ...
        $employees = $this->db->get("under_dev");
        $data = array();
		
		$no = 0;
		foreach($employees->result() as $rows) {
			$no++;
			$url = base_url().'under_dev';
            $data[]= array(
                $no,
                $rows->username,
	...
	public function totalDatas() {
		$query = $this->db->select("COUNT(*) as num")->get("under_dev");
        $result = $query->row();
